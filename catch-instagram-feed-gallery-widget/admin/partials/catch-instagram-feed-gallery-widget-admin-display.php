...
<?php
	$username                                 = '';
	$catch_instagram_feed_gallery_widget_page = admin_url( 'admin.php?page=social-gallery-and-widget' );
...
if ( '' !== $options['access_token'] ) {
	...
	if ( $remaining > -604800 ) {
		...
		$re_data = Catch_Instagram_Feed_Gallery_Widget_Admin::refresh_access_token();
		...
		<div class="catchp_widget_settings">
			<form id="catch-instagram-feed-gallery-widget-main" method="post" action="options.php">
				...
				<div id="dashboard" class="wpcatchtab  nosave active">
					...
					<?php
					settings_fields( 'catch-instagram-feed-gallery-widget-group' );
					$catch_instagram_feed_gallery_widget_options = catch_instagram_feed_gallery_widget_get_options();
					...
					<div id="access-token" class="content-wrapper">
						...
						<div class="content">
							...
							<?php
							$options      = get_option( 'catch_instagram_feed_gallery_widget_options' );
							$username     = isset( $options['username'] ) ? $options['username'] : '';
							$user_id      = isset( $options['user_id'] ) ? $options['user_id'] : '';
							$access_token = isset( $options['access_token'] ) ? $options['access_token'] : '';
							if ( $username && $access_token ) :
								?>
								<p><?php esc_html_e( 'You are logged in as', 'catch-instagram-feed-gallery-widget' ); ?> <strong><?php echo esc_html( $username ); ?></strong></p>
								<p><?php esc_html_e( 'and your access token is', 'catch-instagram-feed-gallery-widget' ); ?> <span class="button show-secure" data-hide="<?php echo esc_html_e( 'Hide', 'catch-instagram-feed-gallery-widget' ); ?>" data-show="<?php esc_html_e( 'Show', 'catch-instagram-feed-gallery-widget' ); ?>"><?php echo esc_html__( 'Show', 'catch-instagram-feed-gallery-widget' ); ?></span></p>
								<p><strong class="dont-break-out secure"><?php echo esc_html( $access_token ); ?></strong></p>
							<?php endif; ?>
							<?php if ( '' === $username || '' === $access_token ) : ?>
								...
								&redirect_uri=https://catchplugins.com/social-gallery-widget-personal?return_url=<?php echo esc_attr( $catch_instagram_feed_gallery_widget_page ); ?>
								&response_type=code
								&state=<?php echo esc_url( $catch_instagram_feed_gallery_widget_page ); ?>"><?php esc_html_e( '+ Connect an Instagram Account', 'catch-instagram-feed-gallery-widget' ); ?></a>

							<?php else : ?>

								<input type="hidden" name="catch-instagram-reset-nonce" id="catch-instagram-reset-nonce" value="<?php echo esc_attr( wp_create_nonce( 'catch-instagram-reset-nonce' ) ); ?>" />
								<a class="button button-large reset-token cifgw-instagram-logout" id="catch-instagram-feed-gallery-widget-reset" href="<?php echo esc_attr( $catch_instagram_feed_gallery_widget_page ) . '&q=logout'; ?>"><?php esc_html_e( 'Logout', 'catch-instagram-feed-gallery-widget-pro' ); ?></a>
			...
			<div id="request-queries" class="content-wrapper">
				<div class="content">
					...
					sprintf(
						wp_kses(
							__( 'If you have any request queries, please visit <a href="%s" target="_blank">this link</a>.', 'catch-instagram-feed-gallery-widget' ),
							array(
								'a' => array(
									'href'   => array(),
									'target' => array(),
